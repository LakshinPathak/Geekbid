...
export default function MyJobsPage() {
 ...
 <div className="bg-[#fbfaf7] h-8 w-8 border-2 border-[#4b3f8f]/40 border-t-[#4b3f8f] rounded-full animate-spin" />
 </div>
 );
 ...
 <div>
 <h1 className="font-heading text-2xl sm:text-3xl font-bold text-[#3d3a45]">My Jobs</h1>
 <p className="text-[#6f6a7d] text-sm mt-1">
 ...
 {/* Tab switcher */}
 <div className="inline-flex w-full overflow-x-auto scrollbar-hide glass-panel-sm rounded-2xl p-1 mb-6">
 {tabs.map(t => (
 <button
 key={t.key}
 ...
 <Briefcase className="h-7 w-7 text-[#6f6a7d]" />
 </div>
 <h3 className="text-lg font-bold text-[#3d3a45] mb-1">No jobs found</h3>
 ...
 <button className="bg-[#4b3f8f] text-[#fbfaf7] font-semibold text-sm px-4 sm:px-6 py-2.5 rounded-full hover:bg-[#3d3373] transition-all">
 ...
 <div
 key={jid}
 className="job-card card-reveal p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-[rgba(75,63,143,0.35)]"
 ...
 <Link href={`/jobs/${jid}`}>
 <h3 className="text-[#3d3a45] font-medium text-base hover:text-[#4b3f8f] transition-colors cursor-pointer truncate">
 {job.title}
 </h3>
 </Link>
 </div>
 <div className="flex flex-wrap items-center gap-4 text-[#6f6a7d] text-xs mt-1">
 ...
 <span className="flex items-center gap-1 text-[#4b3f8f]"><CheckCircle2 className="h-3 w-3" /> Accepted</span>
 )}
 </div>
 </div>
 ...
 <p className="font-heading text-xl font-bold text-[#4b3f8f]">{formatMoney(price)}</p>
 <p className="text-[#6f6a7d] text-[11px]">{isOpen ? "Current price" : "Final price"}</p>
 </div>
 ...
 className="flex items-center gap-1 border border-[rgba(75,63,143,0.22)] text-[#4b3f8f] text-xs font-medium px-4 py-2 rounded-full hover:bg-[rgba(75,63,143,0.12)] transition-all"
 >
 ...
 {/* Review Modal */}
 {reviewModal && (
 <div className="fixed inset-0 bg-[#ffffff]/75 z-50 flex items-center justify-center p-4">
 <div className="glass-panel animate-scale-in p-6 w-full max-w-md" style={{ borderRadius: '6px' }}>
 <div className="flex items-center justify-between mb-4">
 <h3 className="font-heading text-lg font-semibold text-[#3d3a45]">Review {reviewModal.revieweeName}</h3>
 <button onClick={() => setReviewModal(null)} className="text-[#6f6a7d] hover:text-[#3d3a45]"><X className="h-5 w-5" /></button>
 </div>

 <div className="mb-4">
 <label className="text-[#6f6a7d] text-xs font-medium block mb-2">Rating</label>
 <div className="flex gap-1">
 {[1, 2, 3, 4, 5].map(star => (
 <button key={star} onClick={() => setReviewRating(star)} className="transition-transform hover:scale-110">
 <Star className={`h-8 w-8 ${star <= reviewRating ? "text-[#4b3f8f] fill-[#4b3f8f]" : "text-[#6f6a7d]"}`} />
 </button>
 ))}
 </div>
 </div>
 ...
 className="glass-input w-full p-3 rounded-full text-sm resize-none"
 />
 <p className="text-[#6f6a7d] text-xs mt-1">{reviewComment.length}/1000</p>
 </div>

 <div className="flex gap-3">
 <button onClick={() => setReviewModal(null)} className="flex-1 h-10 border border-[rgba(75,63,143,0.22)] text-[#6f6a7d] font-medium rounded-full text-sm hover:bg-[#f4f2ee] hover:text-[#3d3a45] transition-all">
 Cancel
 </button>
 <button
 onClick={async () => {
 ...
 className="flex-1 h-10 btn-primary rounded-full text-sm disabled:opacity-50"
 >
