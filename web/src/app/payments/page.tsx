...
export default function PaymentsPage() {
 ...
 <Loader2 className="h-8 w-8 animate-spin text-[#a8997e]" />
 </div>
 );
 }
 ...
 switch (status) {
 case "released": return "px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-[#050810] text-[#f0e8d4]";
 ...
 <div>
 <h1 className="font-heading text-2xl sm:text-3xl font-bold text-[#f0e8d4]">Financial Terminal</h1>
 <p className="text-[#a8997e] text-sm mt-1">Manage escrow, transactions, and payment methods</p>
 </div>
 {config?.mock && (
 <span className="inline-flex items-center gap-1.5 rounded-full bg-[rgba(201,168,76,0.12)] border border-[rgba(201,168,76,0.25)] px-3 py-1 text-xs font-medium text-[#c9a84c]">
 ...
 <p className="text-[#a8997e] text-xs uppercase tracking-wider font-medium">Total Spent / Earned</p>
 <p className="font-heading text-2xl font-bold text-[#f0e8d4] mt-1 terminal-amount">{formatMoney(totalAmount)}</p>
 </div>
 <div className="finance-card p-5 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
 <p className="text-[#a8997e] text-xs uppercase tracking-wider font-medium">In Escrow</p>
 <p className="font-heading text-2xl font-bold text-[#c9a84c] mt-1 terminal-amount">{formatMoney(heldAmount)}</p>
 </div>
 <div className="finance-card p-5 animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
 <p className="text-[#a8997e] text-xs uppercase tracking-wider font-medium">Available</p>
 <p className="font-heading text-2xl font-bold text-[#c9a84c] mt-1 terminal-amount">{formatMoney(releasedAmount)}</p>
 </div>
 </div>
 ...
 <h2 className="font-heading text-lg font-semibold text-[#f0e8d4]">Fund Escrow</h2>
 </div>
 <p className="text-sm text-[#a8997e] mt-1">Securely pay via Razorpay — funds held in escrow until work is delivered</p>
 </div>

 <div className="p-6 space-y-5">
 <div>
 <label className="block text-sm font-medium text-[#a8997e] mb-1.5">
 Amount ({config?.currency || "INR"})
 </label>
 <div className="relative">
 <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#a8997e]" />
 <input
 type="number"
 min="1"
 step="0.01"
 ...
 placeholder="500.00"
 className="glass-input pl-10 rounded-[6px] terminal-amount"
 />
 </div>
 </div>
 ...
 className="glass-input rounded-[6px]"
 />
 </div>
 ...
 className="btn-primary w-full h-12 rounded-[6px] payment-ready disabled:opacity-40 disabled:cursor-not-allowed"
 >
 {isProcessing ? (
 <>
 ...
 {paymentResult && (
 <div
 className={`rounded-[6px] border p-4 flex items-start gap-3 animate-fade-in-up ${
 paymentResult.success
 ? "bg-[#c9a84c]/5 border-[rgba(201,168,76,0.30)]"
 : "bg-red-500/5 border-[rgba(176,32,32,0.20)]"
 }`}
 >
 {paymentResult.success ? (
 <CheckCircle2 className="h-5 w-5 text-[#c9a84c] mt-0.5 shrink-0" />
 ) : (
 <XCircle className="h-5 w-5 text-[#B02020] mt-0.5 shrink-0" />
 )}
 <div>
 <p className={`text-sm font-medium ${paymentResult.success ? "text-[#c9a84c]" : "text-[#B02020]"}`}>
 {paymentResult.message}
 </p>
 {paymentResult.transactionId && (
 <p className="text-xs text-[#a8997e] mt-1 font-mono-il terminal-amount">
 ...
 <div className="flex items-center gap-2 text-xs text-[#a8997e]">
 ...
 <div className="border-b border-[rgba(201,168,76,0.22)] px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
 <div>
 <h2 className="font-heading text-lg font-semibold text-[#f0e8d4]">Transaction History</h2>
 <p className="text-sm text-[#a8997e] mt-0.5">{typedTransactions.length} transaction{typedTransactions.length !== 1 ? "s" : ""}</p>
 </div>
 ...
 className={`px-3 py-1.5 rounded-[3px] text-xs font-medium transition-all capitalize ${
 statusFilter === f
 ...
 className="ml-1 inline-flex items-center gap-1.5 text-xs font-medium text-[#a8997e] hover:text-[#c9a84c] transition-colors px-3 py-1.5 rounded-[3px] hover:bg-[#111625]"
 >
 ...
 <CreditCard className="h-10 w-10 text-[#a8997e] mx-auto mb-3" />
 <p className="text-sm text-[#a8997e] font-medium">No transactions found</p>
 <p className="text-xs text-[#a8997e] mt-1">
 ...
 <>
 {/* Table header */}
 <div className="hidden sm:grid grid-cols-[1fr_auto_auto_auto_auto_auto] gap-4 px-4 sm:px-6 py-3 bg-[#0d1120]/60 text-[#a8997e] text-xs uppercase tracking-wider font-semibold border-b border-[rgba(201,168,76,0.22)]">
 ...
 <div className="flex h-9 w-9 items-center justify-center rounded-[3px] bg-[#111625] border border-[rgba(201,168,76,0.22)] shrink-0">
 {tx.paymentMethod === "razorpay" ? (
 <CreditCard className="h-4 w-4 text-[#a8997e]" />
 ) : (
 <DollarSign className="h-4 w-4 text-[#a8997e]" />
 )}
 </div>
 <div>
 <p className="text-sm font-medium text-[#f0e8d4]">
 {tx.description || (tx.paymentMethod === "razorpay" ? "Razorpay Payment" : `Job #${(tx.jobId || "").slice(-6)}`)}
 </p>
 <p className="text-xs text-[#a8997e]">{formatDate(tx.createdAt)}</p>
 </div>
 </div>

 {/* Amount */}
 <p className="font-heading text-sm text-[#f0e8d4] text-right w-20 hidden sm:block terminal-amount">
 {formatCurrency(tx.grossAmount)}
 </p>

 {/* Fee */}
 <p className="text-sm text-[#a8997e] text-right w-16 hidden sm:block terminal-amount">
 {tx.platformFee > 0 ? formatCurrency(tx.platformFee) : "—"}
 </p>

 {/* Net */}
 <p className="text-sm font-medium text-[#c9a84c] text-right w-20 hidden sm:block terminal-amount">
 ...
 {tx.mock && (
 <span className="ml-1.5 text-[11px] text-[#a8997e] font-medium">TEST</span>
 )}
 </div>
 ...
 {/* Mobile amounts */}
 <div className="flex items-center gap-4 mt-2 sm:hidden text-xs text-[#a8997e]">
 ...
 {tx.netAmount > 0 && <span className="text-[#c9a84c] terminal-amount">Net: {formatCurrency(tx.netAmount)}</span>}
 </div>
 </div>
 ))}
 </div>
 ...
 <h3 className="font-heading text-lg font-bold text-[#f0e8d4] mb-2">Release Escrow?</h3>
 ...
 return tx ? (
 <p className="text-lg font-heading font-bold text-[#c9a84c] mt-3 mb-6 terminal-amount">
