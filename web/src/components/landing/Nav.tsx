...
export default function Nav() {
  ...
  // Below `md` the link row and "Sign In" that the desktop layout hides
  ...
  // the breakpoint where the full desktop row takes over.
  useEffect(() => {
    ...
    <nav className="sticky top-0 z-50 w-full border-b border-[rgba(91,33,182,0.22)] bg-[#ffffff]">
      ...
      <div className="flex h-14 items-center justify-between px-5 sm:px-8 max-w-[1400px] mx-auto">
        ...
        <div className="flex items-center gap-3">
          <Link href="/login">
            <button className="hidden md:block landing-label text-[#46424e] hover:text-[#5b21b6] transition-colors">Sign In</button>
          </Link>
          ...
          <button
            ...
            className="md:hidden flex items-center justify-center h-10 w-10 -mr-2 text-[#46424e] hover:text-[#5b21b6] transition-colors"
          >
      ...
      {/* Mobile drawer: the same section links + Sign In the desktop row
          hides below `md`, given an actual way back in. Native-<details>-
      ...
      <div
        id="mobile-nav-menu"
        className={`md:hidden grid transition-[grid-template-rows] duration-300 ease-[cubic-bezier(0.2,0.78,0.2,1)] motion-reduce:transition-none ${menuOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
      >
